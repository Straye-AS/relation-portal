...
export function SupplierNotes({
  ...
  const updateNotes = useUpdateSupplierNotes();
      ...
      <Card className={cn("w-full transition-all duration-200", className)}>
        ...
        <CardContent>
          <Textarea
            ...
            placeholder="Skriv interne notater... (Markdown støttes)"
          />
      ...
      >
        {notes ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed py-8 text-muted-foreground transition-colors hover:bg-muted/50">
            <p className="text-sm">Ingen notater lagt til</p>
            {!readOnly && (
              <p className="mt-1 text-xs">Klikk for å legge til notater</p>
            )}
          </div>
        )}
      </CardContent>
