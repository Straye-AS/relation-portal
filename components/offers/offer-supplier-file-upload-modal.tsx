...
interface OfferSupplierFileUploadModalProps {
  offerId: string;
  supplierId: string;
  supplierName?: string;
  open: boolean;
...
export function OfferSupplierFileUploadModal({
  offerId,
  supplierId,
  supplierName,
  open,
  onOpenChange,
}: OfferSupplierFileUploadModalProps) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [isDragging, setIsDragging] = useState(false);
  const [comment, setComment] = useState("");
  const [validationError, setValidationError] = useState<string | null>(null);
  const uploadFile = useUploadOfferSupplierFile();
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {supplierName
              ? `Last opp en fil for ${supplierName}`
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id="comment"
              placeholder="Legg til en kort beskrivelse av filen..."
              value={comment}
